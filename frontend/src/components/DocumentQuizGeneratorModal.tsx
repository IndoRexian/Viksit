...
interface DocumentQuizGeneratorModalProps {
  isOpen: boolean;
  onClose: () => void;
  availableCompetencies?: string[];
}

export const DocumentQuizGeneratorModal: React.FC<
  DocumentQuizGeneratorModalProps
> = ({ isOpen, onClose, availableCompetencies = [] }) => {
  ...
  return (
    ...
    >
      <div
        className={`bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-xl shadow-2xl max-w-3xl w-full my-6 overflow-hidden flex flex-col relative max-h-[92vh] ${
      ...
      >
        <div className="bg-slate-900 text-white px-5 py-4 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-slate-800 border border-slate-700 rounded-lg text-slate-200">
              <FileText size={18} />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-mono uppercase tracking-widest text-slate-400 font-semibold">
                  Official Knowledge Assessment Engine
                </span>
              </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 bg-slate-50/50 dark:bg-slate-950/50 text-slate-900 dark:text-slate-100">
          {error && (
            <div className="mb-5 p-3.5 bg-red-50 dark:bg-red-950/80 border border-red-200 dark:border-red-800 rounded-lg flex items-start gap-3 text-xs text-red-800 dark:text-red-300 animate-fadeIn">
              <AlertCircle
                size={16}
          ...
          {!quizData && !generating && (
            <div key="setup-view" className="space-y-6 animate-fade-in">
              <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-2xs">
                <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
                  Upload any training manual, statistical guideline, survey
                  schedule, or policy document. The Gemini AI engine parses the
                  full document directly, extracting core concepts and
                  generating scientifically calibrated multiple-choice questions
                  aligned with MoSPI official competencies.
                </p>
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-800 dark:text-slate-200 uppercase tracking-wider mb-2 font-mono">
                ...
                <div
                  ...
                  className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all duration-200 ${
                    isDragging
                      ? "border-blue-600 bg-blue-50/60 dark:bg-blue-950/60 scale-[0.99]"
                      : selectedFile
                        ? "border-emerald-500 bg-emerald-50/30 dark:bg-emerald-950/30"
                        : "border-slate-300 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-600 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/60"
                  }`}
                >
                  <input
                    type="file"
                  ...
                  {selectedFile ? (
                    <div className="flex flex-col items-center gap-2">
                      <div className="w-12 h-12 rounded-full bg-emerald-100 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-400 flex items-center justify-center">
                        <FileText size={24} />
                      </div>
                      <div>
                      ...
                      <span className="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-700 dark:text-emerald-300 bg-emerald-100/70 dark:bg-emerald-950/70 px-2.5 py-0.5 rounded-full mt-1">
                        <Check size={12} /> File Ready for Gemini Upload
                      </span>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center gap-2.5 py-2">
                      <div className="w-12 h-12 rounded-full bg-blue-50 dark:bg-blue-950/80 text-blue-900 dark:text-blue-300 border border-blue-100 dark:border-blue-800 flex items-center justify-center">
                        <Upload size={22} />
                      </div>
                      <div>
                        <p className="text-xs sm:text-sm font-medium text-slate-800 dark:text-slate-200">
                          <span className="text-blue-900 dark:text-blue-400 font-semibold underline underline-offset-2">
                            Click to upload
                          </span>{" "}
                ...
                <div className="mt-3 p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 shadow-2xs">
                  <div className="flex items-center gap-2 text-xs text-slate-700 dark:text-slate-300">
                    <span className="text-amber-700 dark:text-amber-400 font-semibold font-mono text-[11px] uppercase">
                      Test Instantly:
                    </span>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {SAMPLE_DOCS.map((sample) => (
                      <button
                        ...
                        className="px-2.5 py-1 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 hover:border-slate-400 text-slate-800 dark:text-slate-200 text-[11px] font-semibold rounded cursor-pointer transition-colors inline-flex items-center gap-1 shadow-2xs"
                        title={sample.title}
                      >
                        <FileText
                          size={11}
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {([5, 10, 15] as const).map((count) => (
                      <button
                        ...
                        className={`py-2.5 px-3 rounded-lg text-xs font-semibold border text-center transition-all cursor-pointer ${
                          numQuestions === count
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={`py-2 px-2.5 rounded-lg text-[11px] font-medium border text-left transition-all cursor-pointer ${
                          difficulty === diff.id
                            ? "bg-slate-900 dark:bg-blue-600 text-white border-slate-900 dark:border-blue-600 shadow-xs"
              ...
              {availableCompetencies.length > 0 && (
                <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-2xs">
                  <label className="block text-xs font-semibold text-slate-800 dark:text-slate-200 uppercase tracking-wider mb-1.5 font-mono">
                    4. Target Competency Focus (Optional)
                  </label>
                  <select
                    value={selectedCompetency}
                    onChange={(e) => setSelectedCompetency(e.target.value)}
                    className="w-full text-xs bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-800 dark:text-white focus:outline-hidden focus:ring-1 focus:ring-blue-900 dark:focus:ring-blue-500 cursor-pointer"
                  >
                    <option value="all">
              ...
              <div className="pt-2">
                <button
                  ...
                  className={`w-full py-3 px-4 rounded-lg font-semibold text-sm flex items-center justify-center gap-2 transition-all cursor-pointer ${
                    selectedFile
                      ? "bg-blue-900 hover:bg-blue-950 text-white shadow-md hover:shadow-lg border border-blue-950"
                      : "bg-slate-200 dark:bg-slate-800 text-slate-400 dark:text-slate-500 cursor-not-allowed border border-slate-300 dark:border-slate-700"
                  }`}
                >
                  <FileText size={16} />
                  <span>Generate Assessment Quiz ({numQuestions} MCQs)</span>
                </button>
              </div>
            </div>
          )}
          ...
          {generating && (
            <div
              key="generating-view"
              className="py-12 flex flex-col items-center justify-center text-center space-y-6 animate-fade-in"
            >
              <div className="relative">
                <div className="w-16 h-16 border-3 border-blue-100 dark:border-blue-950 border-t-blue-900 dark:border-t-blue-500 rounded-full animate-spin" />
                <div className="absolute inset-0 flex items-center justify-center text-blue-900 dark:text-blue-400">
                  <FileText size={20} />
                </div>
              </div>
              <div className="max-w-md space-y-2">
                ...
                <p className="text-xs font-mono text-blue-900 dark:text-blue-300 font-semibold tracking-wide bg-blue-50 dark:bg-blue-950/80 px-3 py-1.5 rounded-full inline-block border border-blue-200 dark:border-blue-800">
          ...
          {quizData && (
            <div key="player-view" className="space-y-5 animate-fade-in">
              {isEvaluating ? (
                /* EVALUATING TRANSITION VIEW */
                <div className="py-20 flex flex-col items-center justify-center text-center space-y-6 animate-fade-in bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs p-8">
                  <div className="relative">
                    <div className="w-16 h-16 border-3 border-emerald-100 dark:border-emerald-950 border-t-emerald-700 dark:border-t-emerald-500 rounded-full animate-spin" />
                    <div className="absolute inset-0 flex items-center justify-center text-emerald-700 dark:text-emerald-400">
                      <Award size={24} />
                    </div>
                  </div>
                  <div className="max-w-md space-y-3">
                    <span className="text-[10px] font-mono uppercase tracking-widest text-emerald-800 dark:text-emerald-300 font-bold bg-emerald-50 dark:bg-emerald-950/80 px-2.5 py-1 rounded border border-emerald-200 dark:border-emerald-800 inline-block">
                    ...
                    <p className="text-xs font-mono text-slate-700 dark:text-slate-300 font-semibold tracking-wide bg-slate-100 dark:bg-slate-800 px-3.5 py-1.5 rounded-full inline-block border border-slate-200 dark:border-slate-700">
                      {evaluationStage ||
                        "Tabulating officer responses against official source standards..."}
                    </p>
                    <div className="w-64 mx-auto bg-slate-200 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden mt-3">
                ...
                <div className="space-y-5">
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-2xs">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={handleReset}
                          className="px-2.5 py-1 text-xs text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded border border-slate-300 dark:border-slate-700 font-medium cursor-pointer transition-colors"
                  ...
                  <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
                    {questions.map((_, idx) => {
                      ...
                      if (isCurrent) {
                        btnColor =
                          "bg-slate-900 dark:bg-blue-600 text-white border-slate-900 dark:border-blue-600 ring-2 ring-blue-300 dark:ring-blue-800";
                      } else if (isAnswered) {
                        btnColor =
                      ...
                      return (
                        <button
                          ...
                          className={`min-w-8 h-8 px-2 rounded-lg text-xs font-mono border transition-all cursor-pointer shrink-0 ${btnColor}`}
                        >
                          {idx + 1}
                        </button>
                      );
                  ...
                  {currentQuestion && (
                    <div
                      key={currentIdx}
                      className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 sm:p-6 shadow-2xs space-y-5 animate-fade-in"
                    >
                      <div className="flex flex-wrap items-center gap-2">
                        <span
                        ...
                        {currentQuestion.competencyTag && (
                          <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 rounded text-[10px] font-mono">
                            {currentQuestion.competencyTag}
                          </span>
                        )}
                      </div>
                      ...
                      <h4 className="text-sm sm:text-base font-semibold text-slate-900 dark:text-white leading-relaxed">
                        <span className="font-mono text-blue-900 dark:text-blue-400 mr-2">
                          Q{currentIdx + 1}.
                        </span>
                      ...
                      <div className="space-y-2.5">
                        {currentQuestion.options.map((opt) => {
                          const isSelected =
                            selectedAnswers[currentIdx] === opt.id;
                          ...
                          if (isSelected) {
                            optionClass =
                              "border-blue-900 dark:border-blue-500 bg-blue-50/70 dark:bg-blue-950/70 text-blue-950 dark:text-blue-100 font-medium ring-1 ring-blue-900 dark:ring-blue-500";
                            badgeClass =
                          ...
                          return (
                            <button
                              ...
                              className={`w-full text-left p-3.5 rounded-lg border transition-all flex items-start gap-3 cursor-pointer ${optionClass}`}
                            >
                              <span
                                className={`w-6 h-6 rounded-md font-mono text-xs uppercase font-bold flex items-center justify-center shrink-0 border ${badgeClass}`}
                              >
                                {opt.id}
                              </span>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800">
                        <button
                          ...
                          className="px-3 py-1.5 border border-slate-300 dark:border-slate-700 rounded-lg text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1 cursor-pointer"
                        >
                        ...
                        {currentIdx < totalQuestions - 1 ? (
                          <button
                            ...
                            className="px-4 py-1.5 bg-slate-900 dark:bg-slate-800 text-white rounded-lg text-xs font-semibold hover:bg-slate-800 dark:hover:bg-slate-700 flex items-center gap-1 cursor-pointer"
                          >
                          ...
                          <button
                            ...
                            className="px-5 py-2 bg-emerald-700 hover:bg-emerald-800 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 shadow-xs cursor-pointer"
                          >
                ...
                <div className="space-y-5 animate-fade-in">
                  <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-xl p-5 shadow-xs space-y-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-full bg-emerald-100 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-400 flex items-center justify-center shrink-0">
                          <Award size={26} />
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-mono uppercase tracking-wider text-emerald-800 dark:text-emerald-300 font-bold">
                              Evaluation Scorecard
                            </span>
                            <span className="px-2 py-0.2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 text-[10px] font-mono rounded">
                              {totalQuestions} Questions Total
                            </span>
                          </div>
                          <h4 className="font-serif text-base sm:text-lg font-bold text-slate-900 dark:text-white mt-0.5">
                            {scorePct >= 80
                      ...
                      <div className="flex items-baseline gap-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-4 py-2 rounded-lg">
                    ...
                    <div className="grid grid-cols-3 gap-2.5 text-center">
                      <div className="p-2.5 bg-emerald-50/50 dark:bg-emerald-950/40 rounded-lg border border-emerald-200 dark:border-emerald-800">
                        <span className="text-[10px] font-mono uppercase text-emerald-800 dark:text-emerald-300 font-semibold block">
                          Correct
                        </span>
                      ...
                      <div className="p-2.5 bg-red-50/50 dark:bg-red-950/40 rounded-lg border border-red-200 dark:border-red-800">
                        <span className="text-[10px] font-mono uppercase text-red-800 dark:text-red-300 font-semibold block">
                          Incorrect
                        </span>
                      ...
                      <div className="p-2.5 bg-blue-50/50 dark:bg-blue-950/40 rounded-lg border border-blue-200 dark:border-blue-800">
                        <span className="text-[10px] font-mono uppercase text-blue-900 dark:text-blue-300 font-semibold block">
                          Accuracy
                        </span>
                        <span className="text-lg font-bold font-mono text-blue-900 dark:text-blue-300">
                          {scorePct}%
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3 rounded-lg shadow-2xs">
                      ...
                      <button
                        ...
                        className={`px-2.5 py-1 text-xs font-semibold rounded border transition-all cursor-pointer ${
                          reviewFilter === "correct"
                            ...
                            : "bg-white dark:bg-slate-800 text-emerald-800 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800 hover:bg-emerald-50 dark:hover:bg-emerald-950/50"
                        }`}
                      >
                        Correct ({score})
                      </button>
                      <button
                        ...
                        className={`px-2.5 py-1 text-xs font-semibold rounded border transition-all cursor-pointer ${
                          reviewFilter === "incorrect"
                            ...
                            : "bg-white dark:bg-slate-800 text-red-800 dark:text-red-300 border-red-200 dark:border-red-800 hover:bg-red-50 dark:hover:bg-red-950/50"
                        }`}
                      >
                    ...
                    {questions
                      ...
                      .map(({ q, originalIdx }) => {
                        ...
                        return (
                          <div
                            key={originalIdx}
                            className={`bg-white dark:bg-slate-900 border rounded-xl p-4 sm:p-5 shadow-2xs space-y-3 transition-all ${
                              isCorrect
                                ? "border-slate-200 dark:border-slate-800 border-l-4 border-l-emerald-600"
                                : isUnanswered
                                  ? "border-slate-200 dark:border-slate-800 border-l-4 border-l-slate-400"
                                  : "border-slate-200 dark:border-slate-800 border-l-4 border-l-red-600"
                            }`}
                          >
                            <div className="flex flex-wrap items-center justify-between gap-2 pb-2.5 border-b border-slate-100 dark:border-slate-800">
                              <div className="flex items-center gap-2">
                                ...
                                {q.competencyTag && (
                                  <span className="px-2 py-0.5 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded text-[10px] font-mono">
                                    {q.competencyTag}
                                  </span>
                                )}
                              </div>
                              ...
                              {isCorrect ? (
                                <span className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-800 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/80 border border-emerald-200 dark:border-emerald-800 px-2.5 py-0.5 rounded">
                                  <CheckCircle2
                                    size={13}
                                ...
                                <span className="inline-flex items-center gap-1 text-xs font-semibold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2.5 py-0.5 rounded">
                                  <span>Unanswered</span>
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1 text-xs font-semibold text-red-800 dark:text-red-300 bg-red-50 dark:bg-red-950/80 border border-red-200 dark:border-red-800 px-2.5 py-0.5 rounded">
                                  <XCircle
                                    size={13}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                              {q.options.map((opt) => {
                                ...
                                if (isThisCorrect) {
                                  optCardStyle =
                                    "bg-emerald-50 dark:bg-emerald-950/80 border-emerald-400 dark:border-emerald-600 text-emerald-950 dark:text-emerald-100 font-semibold ring-1 ring-emerald-400";
                                  optBadgeStyle =
                                    "bg-emerald-600 border-emerald-600 text-white";
                                } else if (isThisSelected && !isThisCorrect) {
                                  optCardStyle =
                                    "bg-red-50 dark:bg-red-950/80 border-red-400 dark:border-red-600 text-red-950 dark:text-red-100 font-semibold ring-1 ring-red-400";
                                  optBadgeStyle =
                                ...
                                return (
                                  <div
                                    key={opt.id}
                                    className={`p-2.5 rounded-lg border text-xs flex items-start gap-2.5 transition-colors ${optCardStyle}`}
                                  >
                                    <span
                                    ...
                                    {isThisCorrect && (
                                      <span className="text-[10px] font-mono font-bold text-emerald-800 dark:text-emerald-300 shrink-0 bg-emerald-100/80 dark:bg-emerald-950/90 px-1.5 py-0.5 rounded">
                                        ✓ Correct
                                      </span>
                                    )}
                                    {isThisSelected && !isThisCorrect && (
                                      <span className="text-[10px] font-mono font-bold text-red-800 dark:text-red-300 shrink-0 bg-red-100/80 dark:bg-red-950/90 px-1.5 py-0.5 rounded">
                                        ✗ Selected
                                      </span>
                                    )}
                                  </div>
                            ...
                            {q.explanation && (
                              <div className="p-3 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-300 space-y-1">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2.5 pt-3 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3.5 rounded-lg border">
                    <button
                      type="button"
                      onClick={handleExportJson}
                      className="px-3.5 py-1.5 border border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-semibold text-xs rounded-lg inline-flex items-center gap-1.5 cursor-pointer transition-colors"
                    >
                      <Download size={13} />
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 font-semibold text-xs rounded-lg inline-flex items-center gap-1.5 cursor-pointer transition-colors"
                      >
                        <RotateCw size={13} />
                      ...
                      <button
                        type="button"
                        onClick={handleReset}
                        className="px-3.5 py-1.5 bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs rounded-lg inline-flex items-center gap-1.5 cursor-pointer transition-colors"
                      >
                        <Upload size={13} />
